...
type CallContext struct {
	...
	// Opcode-scoped key/address cache. cacheGen is incremented once per opcode
	// dispatch in the interpreter loop; cachedKeyGen/cachedAddrGen hold the
	// generation at which the entry was populated. An entry is valid only when
	// its gen equals cacheGen, giving the gas phase and execute phase of the
	// same opcode a shared converted value. Placed before Stack so these fields
	// stay in L1D rather than being pushed out by Stack.data (32 KB).
	cacheGen      uint64
	cachedKeyGen  uint64
...
// peekStorageKey returns the top-of-stack value as a StorageKey.
// The result is cached for the lifetime of one opcode dispatch (gas phase +
// execute phase share the same cacheGen). Callers must invoke this before any
// stack mutation (pop/push/swap) within the same dispatch; the cache is keyed
// by generation only and will not detect a changed stack top within the same
// opcode.
...
// peekAddress returns the top-of-stack value as an Address.
...
func (c *CallContext) put() {
	...
	// always a miss rather than returning a stale value from a prior use.
